from services.reel_extractor import get_reel_video_url
from services.audio_extractor.handler import handle_extract_audio
from services.transcriber.handler import handle_transcribe
from services.transcription_corrector.handler import handle_correct_transcription
import asyncio

async def main():
    url = "https://www.instagram.com/reel/DWBrc8Ij6si"
    cdn_link = "https://scontent.cdninstagram.com/o1/v/t16/f2/m69/AQPK4kM4c772XfeKxBs7787uzgA8d7YYb2v1nnM0ahGnc5xt4aiorcRfGbIbqr1R-QNv95vER7YTFZSdwthwhlg0.mp4?strext=1&_nc_cat=104&_nc_oc=Adp4rDlq1roCeBORCExV5bujzvv12TkKA0LdRDvPovNDAVfqcOfKb1QJLXJF2SRYK774pMoV9LEvzfNi5b7h_Ipn&_nc_sid=5e9851&_nc_ht=instagram.fjlr3-1.fna.fbcdn.net&_nc_ohc=BJYsoXoDolQQ7kNvwHkXeBd&efg=eyJ2ZW5jb2RlX3RhZyI6Inhwdl9wcm9ncmVzc2l2ZS5JTlNUQUdSQU0uQ0xJUFMuQzMuNzIwLmRhc2hfYmFzZWxpbmVfMV92MSIsInhwdl9hc3NldF9pZCI6MTc5NDAxOTg4MTAxNzc0MTMsImFzc2V0X2FnZV9kYXlzIjo1NywidmlfdXNlY2FzZV9pZCI6MTAwOTksImR1cmF0aW9uX3MiOjkxLCJ1cmxnZW5fc291cmNlIjoid3d3In0%3D&ccb=17-1&vs=8986431844e5f206&_nc_vs=&_nc_gid=RQRwIZEh2Hw1vEs7f4yJPw&_nc_zt=28&_nc_ss=7a22e&oh=00_Af6vNtYms9ZUTrettQF7YHr3pVcJsDq0mepI0wX78ovbEA&oe=6A0C6558"
    if not cdn_link:
        cdn_link = get_reel_video_url(url)
    print(f"CDN Link: {cdn_link}")
    audio_extractor_result = await handle_extract_audio({"cdn_link": cdn_link})
    audio = audio_extractor_result
    openai_transcribe_result = await handle_transcribe({
        "audio_bytes_b64": audio.get("audio_bytes_b64"),
        "audio_format": audio.get("audio_format"),
        "transcriber_service": "openai"
    })
    assemblyai_transcribe_result = await handle_transcribe({
        "audio_bytes_b64": audio.get("audio_bytes_b64"),
        "audio_format": audio.get("audio_format"),
        "transcriber_service": "assemblyai"
    })
    print(f"OpenAI Transcription Result: {openai_transcribe_result}")
    print(f"AssemblyAI Transcription Result: {assemblyai_transcribe_result}")

    correct_transcription_result = await handle_correct_transcription({
        "transcripts": [
            openai_transcribe_result.get("transcript_text"),
            assemblyai_transcribe_result.get("transcript_text"),
        ],
    })
    print(f"Corrected Transcription Result: {correct_transcription_result}")
if __name__ == "__main__":
    asyncio.run(main())
