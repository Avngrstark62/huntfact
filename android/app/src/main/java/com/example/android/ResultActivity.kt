...
import com.example.android.ui.components.StatusChip
import com.example.android.ui.components.VerdictChip
...
@Composable
private fun ResultScreen(hunt: com.example.android.hunts.HuntItem) {
...
private fun ClaimRowCard(row: ClaimRow) {
    ...
    Card(
        ...
        Column(
            ...
            Text(
                text = if (expanded) row.explanation else row.explanation.take(220),
                style = MaterialTheme.typography.bodyMedium,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
            )
            if (row.explanation.length > 220) {
                Text(
